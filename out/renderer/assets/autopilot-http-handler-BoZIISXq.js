import { a as autopilotEngine, b as buildEngineStatus, g as googleFlowProvider, r as resolveFlowProjectBinding } from "./autopilot-store-4Sgwsp2L.js";
import { u as useAutoVideoStore } from "./auto-video-store-BurpJGpg.js";
import { h as hasPlanAccess, u as useLicenseStore } from "./index-B8Pnvlyd.js";
import { g as getCapCutVoice } from "./model-registry-CChP-jS9.js";
import { g as getGeminiVoice } from "./gemini-voices-CGiUf3fL.js";
...
async function resolveFlowProject() {
...
function handleJobEvents(jobId, emit) {
  ...
  setTimeout(off, 30 * 60 * 1e3);
}
async function route(request, emit) {
  ...
  if (method === "POST" && path === "/tts") {
    ...
    const modelId = engine === "omnivoice" ? "omnivoice-main" : engine === "vieneu" ? "vieneu-v3-turbo" : engine === "gemini" ? "gemini-3.1-flash-tts-preview" : "capcut-online";
    let capcutVoiceType;
    let capcutResourceId;
    if (engine === "capcut") {
      capcutVoiceType = String(body.voiceType ?? "BV421_vivn_streaming");
      capcutResourceId = getCapCutVoice(capcutVoiceType)?.resourceId || "";
    }
    const geminiVoiceName = engine === "gemini" ? String(body.voiceName ?? "Puck") : void 0;
    ...
    try {
      const result = await window.ttsRuntime?.generate({
        ...
        model: {
          id: modelId,
          repository: engine === "omnivoice" ? "k2-fsa/OmniVoice" : engine === "vieneu" ? "pnnbao97/VieNeu-TTS" : engine === "gemini" ? "https://generativelanguage.googleapis.com" : "https://editor-api-sg.capcutapi.com",
        ...
        capcutResourceId,
        geminiVoiceName,
        vieneuVoice: engine === "vieneu" ? String(body.voiceName ?? "Trúc Ly") : void 0,
        vieneuStyle: engine === "vieneu" ? String(body.style ?? "tu_nhien") : void 0
      });
  ...
  if (method === "POST" && path === "/transcribe") {
    ...
    const apiKey = String(body.apiKey ?? useAutoVideoStore.getState().whisperApiKeys[provider] ?? "");
  ...
  if (method === "POST" && path === "/render") {
    const body = isObject(request.body) ? request.body : {};
    const audioPath = String(body.audioPath ?? "");
    const segments2 = body.segments;
    if (!audioPath || !Array.isArray(segments2) || segments2.length === 0) {
    ...
    const result = await window.autoVideoRuntime?.render({
      ...
      audioPath,
      segments: segments2,
      captionSegments: Array.isArray(body.captionSegments) ? body.captionSegments : void 0,
      mediaMode: body.mediaMode || "image",
  ...
  if (method === "POST" && path === "/flow/video") {
    ...
    try {
      ...
      const result = await googleFlowProvider.generateVideo({
        ...
        startImage: body.startImage ? { source: String(body.startImage), provider: "googleflow", flowProjectId } : void 0,
...
function registerAutopilotHttpHandler() {
