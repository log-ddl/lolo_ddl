import { p as persist } from "./index-ld1jMZXM.js";
import { c as create } from "./zustand-DnVmcEKu.js";
const MAX_TASK_RECORDS = 200;
const useTaskMetadataStore = create()(
  persist(
    (set) => ({
      records: {},
      order: [],
...
function safelyRecord(action) {
  try {
    action();
  } catch (error) {
    console.warn("[TaskMetadata] Could not save task information:", error);
  }
}
const taskMetadata = {
  begin(record) {
    safelyRecord(() => useTaskMetadataStore.getState().begin(record));
  },
  update(id, updates) {
    safelyRecord(() => useTaskMetadataStore.getState().update(id, updates));
  },
  submitted(id, submittedAt = Date.now()) {
    safelyRecord(() => useTaskMetadataStore.getState().update(id, { status: "submitting", submittedAt }));
  },
  completed(id, outputUrl, details) {
...
const rawVoices = voiceCatalog;
const seen = /* @__PURE__ */ new Set();
const CAPCUT_VOICES = rawVoices.map((voice) => ({
  voiceType: voice.voice_type,
  displayName: voice.display_name,
  resourceId: voice.resource_id,
  language: voice.lang,
  languageCode: voice.lan,
  provider: voice.voice_type.includes("Neural") ? "edge" : "capcut"
})).filter((voice) => {
...
const CAPCUT_API_VOICES = CAPCUT_VOICES.filter((voice) => voice.provider === "capcut");
const CAPCUT_LANGUAGES = Array.from(new Set(CAPCUT_API_VOICES.map((voice) => voice.language))).sort();
...
const TTS_MODELS = TTS_MODEL_GROUPS.flatMap((group) => group.models);
function getTtsModel(modelId) {
  return TTS_MODELS.find((model) => model.id === modelId);
}
function getTtsModelGroup(groupId) {
  return TTS_MODEL_GROUPS.find((group) => group.id === groupId);
}
export {
  CAPCUT_API_VOICES as C,
  TTS_MODEL_GROUPS as T,
  getTtsModel as a,
  getTtsModelGroup as b,
  TTS_MODELS as c,
  CAPCUT_LANGUAGES as d,
  getCapCutVoice as g,
  taskMetadata as t,
  useTaskMetadataStore as u
};
