...
let extensionInstanceId = null; // Stable per Chrome profile for multi-account routing
...
function addRequestLog(entry) {
  const existing = requestLog.find((item) => item.id === entry.id);
  if (existing) {
...
function connectToAgent() {
  ...
  ws.onmessage = async ({ data }) => {
    try {
      ...
      } else if (msg.type === 'generation_update') {
