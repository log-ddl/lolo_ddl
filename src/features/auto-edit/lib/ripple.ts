...
/**
 * Ripple editing — ported from opencut-classic (`ripple/{diff,apply,shift}.ts`).
 *
 * After an edit that vacates timeline space (a clip deleted or trimmed shorter),
 * every later clip on the same track slides left to close the gap. This is computed
 * as a pure function over `SceneTracks`: `computeRippleAdjustments` diffs the
 * before/after tracks, producing per-track shift windows, and
 * `applyRippleAdjustments` slides elements left accordingly.
 *
 * Time is integer milliseconds here (opencut used 120000/s ticks); the arithmetic is
 * unit-agnostic so it ports directly.
 */

export interface RippleAdjustment {
  trackId: string;
  /** Shift every element whose startTime >= this boundary left. */
  afterTime: number;
  /** How far left (ms) to shift. */
  shiftAmount: number;
}
...
function collectTrackIntervals({
  ...
  for (const beforeElement of beforeElementsById.values()) {
    ...
    if (!afterElement) {
      // Element deleted. If it was moved to another track (still present elsewhere),
      // the old span isn't truly vacated, so skip it.
    ...
    // Element shortened (trimmed): the tail it gave up is now free.
  ...
  // New elements occupy space, cancelling any vacated interval they cover.
...
function rippleShiftElements({
  ...
  return elements.map((element) =>
    element.startTime >= afterTime
      ? ({ ...element, startTime: element.startTime - shiftAmount } as TimelineElement)
      : element,
  );
}
