...
function DualRange({ label, values, onChange }: { label: string; values: number[]; onChange: (value: number[]) => void }) {
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex items-center gap-2"><input aria-label={`${label} min`} value={formatMetricValue(minValue)} onChange={(event) => { const parsed = parseMetricValue(event.target.value); if (parsed != null) onChange([Math.max(0, Math.min(parsed, maxValue)), maxValue]); }} className="min-w-0 flex-1 rounded-lg border border-border/60 bg-background px-2 py-1.5 text-center text-[11px] font-medium outline-none focus:border-primary" /><span className="text-[11px] text-muted-foreground">—</span><input aria-label={`${label} max`} value={formatMetricValue(maxValue)} onChange={(event) => { const parsed = parseMetricValue(event.target.value); if (parsed != null) onChange([minValue, Math.max(minValue, parsed)]); }} className="min-w-0 flex-1 rounded-lg border border-border/60 bg-background px-2 py-1.5 text-center text-[11px] font-medium outline-none focus:border-primary" /></div>
    </div>
  );
}
...
function PublishedTimeRange({ maxDays, hours, onMaxDaysChange, onHoursChange }: { maxDays: number; hours: number[]; onMaxDaysChange: (days: number) => void; onHoursChange: (hours: number[]) => void }) {
  ...
  return <div>
    <div className="mb-3 flex items-center justify-between gap-2"><p className="text-xs font-semibold">{t("research.discover.published")}</p><label className="flex items-center gap-1.5 text-[10px] text-muted-foreground"><span>{t("research.discover.maxDays")}</span><input type="number" min={1} max={3650} value={maxDays} onChange={(event) => onMaxDaysChange(Number(event.target.value))} className="h-7 w-16 rounded-lg border border-border/60 bg-background px-2 text-center text-[11px] font-medium text-foreground outline-none focus:border-primary" /></label></div>
    ...
    <div className="mt-2 grid grid-cols-[1fr_auto_1fr] items-center gap-2"><label className="rounded-lg border border-border/60 bg-background px-2 py-1.5"><span className="block text-[10px] text-muted-foreground">{t("research.discover.newestAge")}</span><input type="number" min={0} max={Math.max(0, hours[1] - 1)} value={hours[0]} onChange={(event) => setNewest(Number(event.target.value))} className="mt-0.5 w-full bg-transparent text-center text-[11px] font-medium outline-none" /></label><span className="text-[11px] text-muted-foreground">—</span><label className="rounded-lg border border-border/60 bg-background px-2 py-1.5"><span className="block text-[10px] text-muted-foreground">{t("research.discover.oldestAge")}</span><input type="number" min={hours[0] + 1} max={maxHours} value={hours[1]} onChange={(event) => setOldest(Number(event.target.value))} className="mt-0.5 w-full bg-transparent text-center text-[11px] font-medium outline-none" /></label></div>
    <div className="mt-1.5 flex justify-between text-[10px] text-muted-foreground"><span>{formatHourAge(hours[0])}</span><span>{formatHourAge(hours[1])}</span></div>
  </div>;
}
...
function VideoCard({ video, measured, pinned, onTogglePin }: { video: YouTubeVideo; measured: boolean; pinned: boolean; onTogglePin: () => void }) {
  ...
  return (
    <article className="group min-w-0">
      ...
      <a href={`https://www.youtube.com/watch?v=${video.id}`} target="_blank" rel="noopener noreferrer"><h3 className="mt-1.5 line-clamp-2 min-h-10 text-[12px] font-semibold leading-5 transition-colors group-hover:text-primary">{video.title}</h3></a>
      <div className="mt-1 flex min-w-0 items-center gap-1.5 text-[11px] text-muted-foreground"><span className="truncate">{video.channelTitle}</span><span>·</span><span className="shrink-0">{video.subscriberCount ? `${formatViews(video.subscriberCount)} sub` : t("research.discover.hiddenSubs")}</span></div>
      <div className="mt-1.5 flex items-center gap-2 text-[11px] text-muted-foreground"><span className="font-medium text-foreground">{formatViews(video.viewCount)} {t("research.common.views")}</span><span>· {formatAge(video.publishedAt, language)}</span><span className="ml-auto flex items-center gap-0.5"><ThumbsUp className="h-3 w-3" />{formatViews(video.likeCount)}</span><span className="flex items-center gap-0.5"><MessageCircle className="h-3 w-3" />{formatViews(video.commentCount)}</span></div>
    </article>
  );
}
...
export function DiscoverView() {
  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-hidden">
      <aside className="hidden w-[250px] shrink-0 overflow-y-auto border-r border-border/50 bg-panel/40 p-4 xl:block">
        <div className="mb-5 flex items-center gap-2"><SlidersHorizontal className="h-4 w-4" /><h2 className="text-[11px] font-semibold">{t("research.discover.filters")}</h2><button type="button" onClick={resetFilters} className="ml-auto text-[11px] text-primary">{t("research.discover.reset")}</button></div>
        <div className="space-y-6">
          <div><p className="mb-2 text-xs font-semibold">{t("research.discover.contentType")}</p><div className="grid grid-cols-3 rounded-xl bg-muted/40 p-1">{(["Long", "Shorts", "Live"] as VideoKind[]).map((item) => <button type="button" key={item} disabled={loading} onClick={() => changeKind(item)} className={`rounded-lg px-2 py-1.5 text-[11px] font-medium disabled:opacity-50 ${kind === item ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}>{item}</button>)}</div></div>
          {kind === "Long" && <div><p className="mb-2 text-xs font-semibold">{t("research.discover.duration")}</p><div className="grid grid-cols-2 gap-1 rounded-xl bg-muted/40 p-1">{(["any", "short", "medium", "long"] as const).map((item) => <button type="button" key={item} disabled={loading} onClick={() => { setDuration(item); invalidateSearch(); }} className={`rounded-lg px-2 py-1.5 text-[11px] font-medium disabled:opacity-50 ${duration === item ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}>{t(item === "any" ? "research.discover.durationAny" : item === "short" ? "research.discover.durationShort" : item === "medium" ? "research.discover.durationMedium" : "research.discover.durationLong")}</button>)}</div></div>}
        ...
        <div className="mt-6 rounded-xl border border-primary/20 bg-primary/5 p-3 text-[11px] leading-4 text-muted-foreground">{t("research.discover.serverFilterHint")}</div>
        <div className="mt-3 rounded-xl border border-border/50 bg-card/50 p-3 text-[11px] leading-4 text-muted-foreground">{t("research.discover.vphHint")}</div>
      </aside>
      <section className="isolate min-w-0 flex-1 overflow-x-hidden overflow-y-auto px-4 pb-8 lg:px-5">
        <div className="sticky top-0 z-30 -mx-4 mb-5 border-b border-border/50 bg-background px-4 pb-3 pt-4 shadow-sm lg:-mx-5 lg:px-5">
          <form onSubmit={(event) => { event.preventDefault(); void search(); }} className="flex flex-wrap items-center gap-2">
            <div className="relative min-w-[180px] flex-1">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("research.discover.queryPlaceholder")} className="h-10 w-full rounded-xl border border-border/60 bg-card pl-10 pr-3 text-[10px] outline-none focus:border-primary" />
            </div>
            <button type="submit" disabled={loading} className="flex h-10 items-center gap-2 rounded-xl bg-primary px-4 text-xs font-semibold text-primary-foreground disabled:opacity-50">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}{t("research.discover.search")}</button>
            <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
              <span>{t("research.discover.sort")}</span>
              <span className="relative">
                <select aria-label={t("research.discover.sort")} value={sortField} onChange={(event) => setSortField(event.target.value as SortField)} className="h-10 min-w-32 cursor-pointer appearance-none rounded-xl border border-border/60 bg-card pl-3 pr-8 text-xs font-semibold text-foreground outline-none focus:border-primary">
            ...
            <button type="button" onClick={() => setSortDir(sortDir === "desc" ? "asc" : "desc")} title={t(sortDir === "desc" ? "research.discover.sortDesc" : "research.discover.sortAsc")} className="flex h-10 shrink-0 items-center gap-1.5 rounded-xl border border-border/60 bg-card px-3 text-xs font-medium">{sortDir === "desc" ? <ArrowDown className="h-4 w-4" /> : <ArrowUp className="h-4 w-4" />}<span className="hidden sm:inline">{t(sortDir === "desc" ? "research.discover.sortDesc" : "research.discover.sortAsc")}</span></button>
          </form>
          <div className="mt-2.5 flex flex-wrap items-center gap-2 text-[11px]"><span className="flex items-center gap-1 rounded-full bg-muted/50 px-2.5 py-1">{t("research.discover.type")} <strong>{kind}</strong></span><span className="rounded-full bg-muted/50 px-2.5 py-1">{t("research.discover.time")} <strong>{timeLabel}</strong></span><span className="rounded-full bg-muted/50 px-2.5 py-1">Views: <strong>{formatMetricValue(viewValues[0])}–{formatMetricValue(viewValues[1])}</strong></span><span className="rounded-full bg-muted/50 px-2.5 py-1">Sub: <strong>{formatMetricValue(subValues[0])}–{formatMetricValue(subValues[1])}</strong></span><button type="button" onClick={resetFilters} className="font-medium text-primary">{t("research.discover.clearAll")}</button></div>
          {sortField === "composite" && <div className="mt-2.5 rounded-xl border border-border/60 bg-card/60 p-3">
            <div className="flex flex-wrap items-center gap-1.5"><span className="text-[11px] font-semibold text-muted-foreground">{t("research.discover.compositePresets")}</span>{COMPOSITE_PRESETS.map((preset) => <button type="button" key={preset.key} onClick={() => setWeights(preset.weights)} className="rounded-full bg-muted/60 px-2.5 py-1 text-[11px] font-medium hover:bg-primary/10 hover:text-primary">{t(preset.labelKey)}</button>)}</div>
            <div className="mt-2.5 grid gap-x-4 gap-y-2 sm:grid-cols-2">{COMPOSITE_SIGNALS.map((signal) => <label key={signal} className={`flex items-center gap-2 ${weights[signal] === 0 ? "opacity-45" : ""}`}><span className="w-20 shrink-0 text-[11px] font-medium">{t(COMPOSITE_SIGNAL_LABEL[signal])}</span><input type="range" min={0} max={100} step={10} value={weights[signal]} onChange={(event) => setWeights((current) => ({ ...current, [signal]: Number(event.target.value) }))} className="h-1 flex-1 accent-primary" /><span className="w-8 shrink-0 text-right text-[11px] tabular-nums text-muted-foreground">{weights[signal]}%</span></label>)}</div>
            <p className="mt-2 text-[10px] leading-3 text-muted-foreground">{t("research.discover.compositeHint")}</p>
          </div>}
          {error && <p className="mt-2 flex items-center gap-1.5 text-[11px] text-destructive"><AlertCircle className="h-3.5 w-3.5" />{error}</p>}
        </div>
