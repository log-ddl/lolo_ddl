  ...
  return (
    ...
    <section className="flex h-[88%] max-h-[820px] w-[92%] max-w-[1500px] flex-col overflow-hidden rounded-2xl border border-border bg-background shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
      <header className="flex h-12 shrink-0 items-center gap-3 border-b border-border px-3">
        <span className="flex size-7 items-center justify-center rounded-lg bg-primary/10 text-primary">{inputNode ? inputNode.kind === "text" ? <Type className="size-4" /> : inputNode.kind === "file" ? <FileText className="size-4" /> : <FolderOpen className="size-4" /> : functionNode ? functionNode.kind === "condition" ? <GitBranch className="size-4" /> : functionNode.kind === "loop" ? <Repeat2 className="size-4" /> : <Merge className="size-4" /> : <Bot className="size-4" />}</span>
        <div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">{title}</p><p className="text-2xs text-muted-foreground">{family}</p></div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_360px_minmax(0,1fr)]">
        <section className="flex min-w-0 flex-col border-r border-border bg-muted/10">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-4">
            {inputSources.length > 0 ? <div className="mx-auto w-full max-w-md space-y-3"><div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm"><div className="flex items-center justify-between border-b border-border bg-muted/25 px-3 py-2"><span className="text-xs font-semibold">Input</span><span className="text-2xs text-muted-foreground">{inputSources.length} {vietnamese ? "mục" : "items"}</span></div>{inputSources.map((source) => {
              ...
              return <button key={source.id} type="button" draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = "copy"; event.dataTransfer.setData("text/plain", source.token); }} onDoubleClick={() => insertPromptToken(source.token)} className="group flex w-full cursor-grab items-center gap-2 border-b border-border/60 px-3 py-2.5 text-left last:border-b-0 hover:bg-muted/25 active:cursor-grabbing"><GripVertical className="size-3.5 shrink-0 text-muted-foreground/50" /><SourceIcon className="size-3.5 shrink-0 text-muted-foreground" /><span className="min-w-0 flex-1"><span className="block text-2xs font-medium">{label}</span><span className="block text-2xs text-muted-foreground">{source.field}</span></span><span className="max-w-[48%] truncate rounded-lg bg-muted px-2 py-1 text-2xs text-muted-foreground" title={source.value}>{source.value || (vietnamese ? "Chưa có dữ liệu" : "No data")}</span><span className="rounded bg-primary/10 px-1.5 py-0.5 font-mono text-2xs text-primary">{source.dataType}</span></button>;
            })}</div><p className="px-1 text-2xs leading-4 text-muted-foreground">{vietnamese ? "Kéo một mục sang ô Chỉ dẫn, hoặc nhấp đúp để chèn." : "Drag an item into Prompt, or double-click to insert it."}</p></div> : <div className="flex h-full items-center justify-center text-center"><div><p className="text-xs font-medium">{vietnamese ? "Không có dữ liệu đầu vào" : "No input data"}</p><p className="mt-1 text-2xs text-muted-foreground">{vietnamese ? "Đóng cửa sổ và kéo dây từ node Input vào Agent." : "Close this window and connect an Input node to the Agent."}</p></div></div>}
          </div>
        </section>
            ...
            {editorTab === "settings" ? <>
              ...
              {agentNode && selectedAgentPreset && <>
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1"><Label className="text-2xs">{vietnamese ? "Tên Agent" : "Agent name"}</Label><Input value={agentDraft.name} maxLength={60} onChange={(event) => setAgentDraft((draft) => ({ ...draft, name: event.target.value }))} /></div>
                  <div className="space-y-1"><Label className="text-2xs">{vietnamese ? "Vai trò" : "Role"}</Label><Input value={agentDraft.role} maxLength={120} placeholder={vietnamese ? "Ví dụ: nghiên cứu" : "Example: researcher"} onChange={(event) => setAgentDraft((draft) => ({ ...draft, role: event.target.value }))} /></div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div className="space-y-1"><Label className="text-2xs">CLI</Label><Select value={agentDraft.adapter} onValueChange={(value) => setAgentDraft((draft) => ({ ...draft, adapter: value as BuzzAdapter, model: "", effort: "" }))}><SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger><SelectContent>{(Object.keys(ADAPTER_LABELS) as BuzzAdapter[]).map((adapter) => <SelectItem key={adapter} value={adapter}>{ADAPTER_LABELS[adapter]}</SelectItem>)}</SelectContent></Select></div>
                  <div className="space-y-1"><Label className="text-2xs">Model</Label><Select value={agentDraft.model || "__default"} onValueChange={(value) => setAgentDraft((draft) => ({ ...draft, model: value === "__default" ? "" : value }))} disabled={loadingAgentOptions}><SelectTrigger className="h-8 text-xs">{loadingAgentOptions ? <Loader2 className="size-3 animate-spin" /> : <SelectValue />}</SelectTrigger><SelectContent><SelectItem value="__default">{vietnamese ? "Mặc định" : "Default"}</SelectItem>{agentModels.map((model) => <SelectItem key={model} value={model}>{model}</SelectItem>)}</SelectContent></Select></div>
                  <div className="space-y-1"><Label className="text-2xs">{vietnamese ? "Suy luận" : "Reasoning"}</Label><Select value={agentDraft.effort || "__default"} onValueChange={(value) => setAgentDraft((draft) => ({ ...draft, effort: value === "__default" ? "" : value }))} disabled={loadingAgentOptions || agentEfforts.length === 0}><SelectTrigger className="h-8 text-xs">{loadingAgentOptions ? <Loader2 className="size-3 animate-spin" /> : <SelectValue />}</SelectTrigger><SelectContent><SelectItem value="__default">{vietnamese ? "Mặc định" : "Default"}</SelectItem>{agentEfforts.map((effort) => <SelectItem key={effort} value={effort}>{effort}</SelectItem>)}</SelectContent></Select></div>
                </div>
                <div className="space-y-1"><Label className="text-2xs">System prompt</Label><Textarea value={agentDraft.systemPrompt} onChange={(event) => setAgentDraft((draft) => ({ ...draft, systemPrompt: event.target.value }))} placeholder={vietnamese ? "Agent này là ai, làm gì, và không được làm gì?" : "Who is this Agent, what does it do, and what must it avoid?"} className="min-h-36 resize-y font-mono text-2xs leading-4" /></div>
                <Button type="button" size="sm" className="w-full" onClick={saveAgentSettings}>{vietnamese ? "Lưu cài đặt Agent" : "Save Agent settings"}</Button>
              </>}
              ...
              {functionNode?.kind === "condition" && <>
                ...
                <div className="space-y-1.5"><Label className="text-xs">Giới hạn số lần đánh giá</Label><Input type="number" min={1} max={60} value={functionNode.maxIterations ?? 10} onChange={(event) => onUpdateFunction(functionNode.id, { maxIterations: Math.max(1, Math.min(60, Number(event.target.value) || 1)) })} /><p className="text-2xs text-muted-foreground">Chặn dây quay ngược lặp vô hạn khi điều kiện không bao giờ đổi.</p></div>
              </>}
              {functionNode?.kind === "loop" && <>
                <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-3 text-2xs leading-5 text-muted-foreground"><Repeat2 className="mr-1 inline size-3.5 text-cyan-500" />Mỗi lần nhận dữ liệu, node đi qua cổng Lặp. Khi đủ lượt, nó chuyển sang cổng Xong.</div>
                <div className="space-y-1.5"><Label className="text-xs">Số lượt quay lại</Label><Input type="number" min={1} max={60} value={functionNode.maxIterations ?? 3} onChange={(event) => onUpdateFunction(functionNode.id, { maxIterations: Math.max(1, Math.min(60, Number(event.target.value) || 1)) })} /><p className="text-2xs text-muted-foreground">Ví dụ 3 nghĩa là chạy nhánh Lặp tối đa 3 lần, lần tiếp theo đi qua nhánh Xong.</p></div>
              </>}
              {agentNode && <>
        ...
        <section className="flex min-w-0 flex-col bg-muted/10">
          <h3 className="border-b border-border px-4 py-3 text-2xs font-semibold text-muted-foreground">{vietnamese ?"Đầu ra" :"Output"}</h3>
          <div className="flex min-h-0 flex-1 items-center justify-center p-5">
            {output ? <pre className="max-h-full w-full overflow-auto whitespace-pre-wrap break-words rounded-lg border border-border bg-card p-3 font-mono text-2xs leading-5">{output}</pre> : <div className="text-center"><p className="text-xs font-medium">Chưa có dữ liệu đầu ra</p><p className="mt-1 text-2xs text-muted-foreground">{agentNode ? (vietnamese ? "Chạy node để xem kết quả." : "Execute the node to see its output.") : (vietnamese ? "Chạy quy trình để xem kết quả." : "Execute the workflow to see its output.")}</p></div>}
          </div>
        </section>
      </div>
    </section>
