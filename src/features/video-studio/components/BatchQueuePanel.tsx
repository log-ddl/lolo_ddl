...
export function BatchQueuePanel() {
  ...
  return (
    <div className="mb-6 rounded-xl border border-border/60 bg-card/80 shadow-sm">
      ...
      {waiting && (
        <div className="border-b border-border/60 bg-amber-500/5 px-4 py-1.5 text-xs text-amber-600 dark:text-amber-400">
          {waitingForSchedule
    ...
    <div className="flex items-center gap-3 px-4 py-2.5">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {entry.scheduledAt && (
            <span className="flex shrink-0 items-center gap-1 rounded bg-primary/10 px-1.5 py-0.5 text-2xs text-primary">
        ...
        {(entry.status === "running" || entry.progress > 0) && (
          <div className="mt-1 flex items-center gap-2">
            ...
            <span className="shrink-0 text-2xs tabular-nums text-muted-foreground">{Math.round(entry.progress)}%</span>
          </div>
        )}
        {(entry.status === "running" || (entry.completedSteps?.length ?? 0) > 0) && <StageTimeline entry={entry} />}
        <div className={cn("mt-0.5 truncate text-2xs", entry.status === "failed" ? "text-red-500" : "text-muted-foreground")}>
  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent align="end" className="w-72 space-y-3">
        <div className="text-xs font-semibold">Hẹn giờ chạy</div>
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-2 text-center text-xs">
          Chạy lúc:{" "}
          <span className="font-semibold text-primary">{preview ? formatSchedule(preview) : "—"}</span>
          {isPast && <div className="mt-0.5 text-2xs text-amber-600 dark:text-amber-400">Giờ đã qua — sẽ chạy ngay khi tới lượt.</div>}
        </div>
