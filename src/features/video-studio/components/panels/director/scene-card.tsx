...
import type { AIScene, SceneProgress } from "@/features/video-studio/packages/ai-core";
      ...
      case "video":
        return (
          <span className="flex items-center gap-1 text-xs text-muted-foreground">
            ...
            {t("director.generateVideo")}
          </span>
        );
      case "audio":
        return (
          <span className="text-xs text-muted-foreground">
            {t("director.generateAudio")}
          </span>
        );
      case "done":
        return (
          <span className="text-xs text-green-500">
            {t("director.completedStatus")}
          </span>
        );
      default:
        return null;
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          {getStatusIcon()}
            ...
            {t("director.sceneNumber", { id: scene.sceneId })}
          </span>
          {/* Mood badge */}
          {isPreview && !progress && (
            <Popover open={moodPopoverOpen} onOpenChange={setMoodPopoverOpen}>
              ...
              <PopoverContent className="w-56 p-2" onClick={(e) => e.stopPropagation()}>
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("director.chooseEmotionTags")}</div>
      ...
      {isEditingNarration ? (
        <div className="mb-2" onClick={(e) => e.stopPropagation()}>
          <Textarea
            ...
            placeholder={t("director.sceneNarrationPlaceholder")}
          />
        ...
        <p 
          ...
          title={isPreview && !progress ? t("director.doubleClickEdit") : undefined}
        >
          {scene.narration}
        </p>
      )}
      ...
      {showImage && (progress?.imageUrl || scene.imageUrl) && (
        <div className="mt-2 relative">
          <img
            ...
            alt={t("director.sceneNumber", { id: scene.sceneId })}
      ...
      {isSelected && (
        <div className="mt-3 pt-3 border-t space-y-2">
          <div>
            <label className="text-xs font-medium text-muted-foreground">{t("director.shotLabel")}</label>
            <p className="text-xs">{scene.camera}</p>
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground">{t("director.actionLabel")}</label>
