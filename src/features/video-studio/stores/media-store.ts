...
import { storageService } from "@/features/video-studio/lib/storage/storage-service";
import { generateUUID } from "@/shared/lib/utils";
import { MediaType, MediaFile, MediaFolder, MediaFolderCategory } from "@/features/video-studio/types/media";
import { saveImageToLocal, isElectron } from "@/features/video-studio/lib/image-storage";
import type { ImageCategory } from "@/features/video-studio/lib/image-storage";
...
export const useMediaStore = create<MediaStore>()(
  ...
  addMediaFromUrl: ({ url, name, type, source, thumbnailUrl, duration, folderId, projectId }) => {
    ...
    const isTemporaryUrl = (value?: string) => Boolean(value && (value.startsWith('data:') || value.startsWith('blob:') || value.startsWith('http')));
      ...
      (async () => {
        try {
          ...
          set((state) => {
            ...
            return {
              mediaFiles: exists
                ? state.mediaFiles.map((f) => f.id === id ? persistedItem : f)
                : [...state.mediaFiles, persistedItem],
            };
          });
        } catch (error) {
  ...
  initSystemFolders: () => {
    ...
    // Migrate the legacy AI-generated folder into the system ai-image folder.
        ...
        set((state) => ({
          folders: state.folders.map((f) =>
            f.id === legacyAiFolder.id
              ? { ...f, name: 'AI Images', isSystem: true, category: 'ai-image' as const, projectId: undefined }
              : f
          ),
        }));
    ...
    // Also migrate legacy timestamp folders ("Project-MM-DD HH:MM") to remove orphans.
    // Files in those folders will be moved into the ai-image folder later by user action.
  ...
  assignProjectToUnscoped: (projectId) => {
    ...
    }),
    {
      ...
      storage: createJSONStorage(() => createSplitStorage<MediaPersistedState>(
        'media', splitMediaData, mergeMediaData, 'shareMedia', true
      )),
      partialize: (state) => ({
      ...
      onRehydrateStorage: () => (state) => {
        if (!state) return;
        // Initialize system folders (AI Images, AI Videos, Uploads)
