      ...
      if (!imagePath) {
        ...
        const result = (await runWithModelFallback(
          ...
          (model, modelIndex) => generateImageWithSelectedProvider({
            ...
            onSubmitted: (submittedAt) => ctx.updateCharacterOutput(job.id, name, { status: 'generating', imageSubmittedAt: submittedAt ?? Date.now() }),
            signal,
          }),
      ...
      if (!imagePath) {
        ...
        const result = (await runWithModelFallback(
          ...
          (model, modelIndex) => generateImageWithSelectedProvider({
            ...
            onSubmitted: (submittedAt) => ctx.updateSceneOutput(job.id, name, { status: 'generating', imageSubmittedAt: submittedAt ?? Date.now() }),
            signal,
          }),
