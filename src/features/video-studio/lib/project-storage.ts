...
export function createProjectScopedStorage(storeName: string): StateStorage {
  return {
    getItem: async (name: string): Promise<string | null> => {
      // Wait for project-store hydration so reads use the correct activeProjectId.
      // Otherwise startup can fall back to "default-project" and read the wrong file.
...
export function createSplitStorage<T = any>(
  ...
  alwaysMergeShared = false,
): StateStorage {
  return {
    getItem: async (name: string): Promise<string | null> => {
      // Wait for project-store hydration before resolving the active project ID.
      if (!useProjectStore.persist.hasHydrated()) {
        await new Promise<void>((resolve) => {
          const unsub = useProjectStore.persist.onFinishHydration(() => {
            unsub();
    ...
    setItem: async (name: string, value: string): Promise<void> => {
      ...
      try {
        ...
        // Collect unique projectIds from the state that still belong to a known
        // project. Legacy data can contain stale item projectIds; writing those
        // IDs would recreate resource-only _p/{id} folders that disk discovery
        // could later mistake for standalone projects.
