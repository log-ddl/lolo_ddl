...
/**
 * Predefined optimal layouts to keep AI generation and splitting consistent.
 * Principle: prefer near-square grids so each cell is closer to the target aspect ratio.
 */
...
export function calculateGrid(input: GridCalculatorInput): GridConfig {
  ...
  // Check whether a predefined optimal layout exists.
  ...
  if (optimalLayout) {
    ...
    // Calculate cell size while keeping the target aspect ratio.
  ...
  // All other scene counts: use dynamic calculation.
  if (aspectRatio === '16:9') {
...
export function validateSceneCount(sceneCount: number, resolution: Resolution): {
  ...
  return {
    ...
    message: isValid 
      ? '' 
      : `Scene count exceeds the ${resolution} resolution limit (max ${limit}). Switch to a higher resolution or reduce the scene count.`,
  };
}
