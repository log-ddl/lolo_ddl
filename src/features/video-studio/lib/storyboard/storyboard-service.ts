 ...
 * 
 * Handles the generation of storyboard grid images using AI image APIs.
 * For Electron desktop app: directly calls external APIs
 */
...
import { delay, RATE_LIMITS } from "@/features/video-studio/lib/utils/rate-limiter";
...
async function pollTaskCompletion(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      // Always use the caller-provided baseUrl instead of a hardcoded value.
...
export async function generateStoryboardImage(
  ...
  if (result.taskId) {
    // Poll using the same baseUrl that was used to submit the task.
      ...
      if (provider !== 'zhipu') {
        ...
        if (result.taskId) {
          const videoUrl = await pollVideoTaskCompletion(
            result.taskId,
            apiKey,
            resolvedBaseUrl, // Use the same baseUrl that was used when submitting the task
            (progress) => {
