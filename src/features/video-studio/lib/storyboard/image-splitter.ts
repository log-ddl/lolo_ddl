 ...
 * 
 * Uses the FIXED UNIFORM GRID approach (Approach D):
...
export async function splitStoryboardImage(
  ...
  // === Dynamic center-crop correction (adapted from merged-generation splitting) ===
  ...
  // Calculate the actual aspect ratio of each source cell
  const rawRatio = cellWidth / cellHeight;
  
  // Calculate crop parameters and apply center-crop correction when ratios mismatch
  let cropX = 0, cropY = 0, cropW = cellWidth, cropH = cellHeight;
  let outputWidth: number, outputHeight: number;
  
  if (Math.abs(rawRatio - targetRatio) < 0.01) {
    // The aspect ratio is already close enough to the target; use it directly.
    ...
    // The source cell is too wide; crop width from the center.
    ...
    // The source cell is too tall; crop height from the center.
  ...
  // Safety net: force the output dimensions to match the target aspect ratio exactly.
  if (aspectRatio === '16:9') {
  ...
  // Calculate safety margin (inset) inside the cropped region.
  ...
  for (let i = 0; i < cellDefs.length; i++) {
    ...
    // Calculate the source rectangle with crop + inset.
    // Apply the center-crop offset first, then apply the safety inset.
