...
import { useSimpleTimelineStore } from '@/features/video-studio/stores/simple-timeline-store';
...
export async function switchProject(newProjectId: string): Promise<void> {
  ...
  // Auto-video transient state (active job ids, progress, errors, log) is in-memory only —
  // rehydrate() does not touch it, so reset it here to avoid showing the previous project's
  // render/transcribe progress against the new project's data.
