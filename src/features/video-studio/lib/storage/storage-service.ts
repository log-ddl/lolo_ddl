import { TProject } from "@/features/video-studio/types/project";
...
import { TimelineTrack } from "@/features/video-studio/types/timeline";
import { SavedSoundsData, SavedSound, SoundEffect } from "@/features/video-studio/types/sounds";
...
class StorageService {
  ...
  async deleteProject({ id }: { id: string }): Promise<void> {
    // Clean media and timeline data linked to the project first to avoid orphaned records.
    try {
      await Promise.all([
      ...
      // Continue deleting the project metadata even if cleanup fails to avoid blocking the user.
    }
  ...
  async clearAllData(): Promise<void> {
    // Load all project ids first, then clean media and timeline data for each project.
    try {
      const projectIds = await this.projectsAdapter.list();
      await Promise.all(
        projectIds.map(id =>
    ...
    // Finally clear the project metadata.
