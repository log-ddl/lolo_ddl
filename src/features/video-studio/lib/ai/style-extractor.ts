...
- summary is a 1-2 sentence English summary of the overall style
...
function extractErrorMessage(status: number, errorText: string): string {
  ...
  if (status === 401 || status === 403) {
    return 'API key is invalid or expired. Please check the image understanding service key configuration.';
  }
...
export async function extractStyleTokens(
  ...
  if (!config) {
    throw new Error('Please bind an API provider for image understanding in settings first');
  }

  const baseUrl = config.baseUrl?.replace(/\/+$/, '');
  const model = config.model || config.models?.[0];
  if (!baseUrl || !model) {
    throw new Error('Image understanding service is missing a Base URL or model configuration');
  }
  ...
  try {
    ...
    throw new Error('AI returned a response format that could not be parsed');
  }
