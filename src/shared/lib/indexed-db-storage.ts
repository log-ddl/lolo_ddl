...
const hasRichData = (jsonStr: string | null): boolean => {
  ...
  // Mọi nhánh bên dưới hoặc trả true, hoặc rơi xuống `length > 1000`. Nên khi đã
  // dài hơn 1000 thì kết quả chắc chắn là true — chốt sớm để khỏi JSON.parse cả
  // file 500 KB ở mỗi lần khởi động.
  if (jsonStr.length > 1000) return true;
  try {
  ...
  getItem: async (name: string): Promise<string | null> => {
    ...
    if (isElectron()) {
      try {
        ...
        // Đường đi thường ngày của một máy đã dùng lâu. Không đụng tới IndexedDB ở
        // đây nữa: mở kết nối IndexedDB tốn hàng chục ms và nằm chắn trước lúc
        // hydrate, mà dữ liệu cũ trong đó thì đã được dọn từ lần chạy trước.
        if (fileHasData) {
          if (localData) localStorage.removeItem(name);
          // Dọn tồn dư trong nền, không chắn đường đọc.
          void purgeStaleIndexedDB(name);
          return fileData;
        }

        // Chỉ khi cả file lẫn localStorage đều không có gì mới cần soi IndexedDB.
...
/**
 * Dọn bản sao cũ trong IndexedDB, chạy nền và chỉ một lần cho mỗi khoá mỗi phiên.
 * Không await ở đường đọc, vì mở IndexedDB là thứ chắn trước lúc hydrate.
 */
const purgedIndexedDBKeys = new Set<string>();
const purgeStaleIndexedDB = async (name: string): Promise<void> => {
  ...
  try {
    ...
    // IndexedDB không dùng được thì thôi, đây chỉ là dọn dẹp.
  }
};
