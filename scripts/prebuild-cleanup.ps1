...
foreach ($process in Get-Process -Name "video_creator", "logdd" -ErrorAction SilentlyContinue) {
...
foreach ($process in Get-CimInstance Win32_Process -ErrorAction SilentlyContinue) {
  ...
  $name = $process.Name
  $isBuilderProcess = Test-IsPathUnderDirectory -Path $exePath -Directory $normalizedBuilderOutputDir
  $isSetupProcess = $name -and $name -like "video_creator-*-setup.exe" -and (Test-IsPathUnderDirectory -Path $exePath -Directory $normalizedReleaseDir)
  if ($isBuilderProcess -or $isSetupProcess) {
